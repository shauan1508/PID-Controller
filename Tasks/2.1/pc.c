...
float run_time = 10; // Total runtime in seconds
int motor_number = 1; // Motor number (update based on your setup)
...
int main() {
    ...
    while (1) {
        ...
        printf("u: Select input type (Step or Square)\n");
        ...
        switch (selection) {
            ...
            case 'u': // Choose input type (Step or Square)
                ...
                for (int i = 0; i < MAXS; i++) {
                    ref[i] = magnitude;  // Basic step response
                }
                break;
...
void *ControlLoop(void *arg) {
    ...
    while (k < no_of_samples) {
        ...
        DtoA(VtoD(control_signal));  // Send signal to D/A converter
